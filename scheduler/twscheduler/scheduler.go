...
func NewScheduler(name string, tick time.Duration, slotNum int) schedulerapi.Scheduler {
	if tick <= 0 {
		panic("tick must > 0")
	}
	if slotNum <= 0 {
		panic("slotNum must > 0")
	}
	slotNum = 1 << bits.Len(uint(slotNum)-1) // 确保 slotNum 是 2 的幂
	s := &scheduler{
		name:    name,
		tick:    tick,
